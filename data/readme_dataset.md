...
Ce dossier contient l'ensemble des données extraites de l'API de Warframe Market (WFM) par le script de scraping. L'architecture est optimisée pour le *Lazy Loading* (chargement asynchrone) afin de minimiser le poids des fichiers initiaux chargés par le client Web.
...
Fichiers lourds structurés sous forme de dictionnaire (`clé: valeur`) où la clé est l'identifiant (`slug`) de l'objet. Ils contiennent les données textuelles (traductions, descriptions, liens) et la structure interne de l'objet (ses sous-composants avec leurs propres statistiques). Ils sont chargés à la demande (ex: au survol ou au clic sur un Set).
...
Note : Pour les catégories sans composants (Mods, Arcanes, Reliques), la liste "components" est présente mais vide [].

📈 Signification des Indicateurs Économiques
Chaque objet (et chaque composant interne) dispose d'un set d'indicateurs standardisés calculés à partir de l'historique des prix des dernières 48 heures et des 90 derniers jours :
...
p90 / float / Évolution sur 90 jours (%) / Variation en pourcentage entre le prix d'équilibre actuel (p) et la moyenne mobile d'il y a 90 jours. / vintVolume RécentQuantité totale de l'objet échangée (acheté/vendu) au cours des dernières 48 heures.
...
Pour les objets possédant un système de progression (comme les Mods ou les Arcanes), la fonction de calcul sépare strictement les données. Le dictionnaire inclut alors les indicateurs pour le Rang 0 (clés de base : p, v, etc.) ET le Rang Maximum via des clés suffixées _max :p_max, p90_max, v_max, vr_max, ds_max, f_max
